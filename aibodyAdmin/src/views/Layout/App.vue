      ...
      <div class="aside">
        <div class="menu">
          <el-menu
              ...
              <el-menu-item v-else :index="item.href" :key="item.id">
                <i :class="item.icon"></i>
                <span slot="title">{{ item.name }}</span>
              </el-menu-item>
...
export default {
  data() {
    // let MenuNew = {};
    // this.$axios.get("/admin/getMenu").then(response => {
    //   if (response.data.code == 200) {
    //     MenuNew.data = response.data.data;
    //     console.log(MenuNew);
    //   }
    // });
    return {
  ...
  created() {
    this.$axios.get("/admin/getMenuTreeData").then(response => {
      if (response.data.code == 200) {
